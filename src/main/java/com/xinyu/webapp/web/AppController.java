...
import com.xinyu.webapp.security.basicauth.entity.UserNotFoundException;
...
public class AppController {
  ...
  public ResponseEntity<String> updateUser(@PathVariable(value = "id") int id,
    ...
    if (!userOri.getUsername().equals(authentication.getName())) {
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Can not update other account's information");
    }
    if (!userOri.getUsername().equals(user.getUsername())) {
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Can not update account's username");
    }
